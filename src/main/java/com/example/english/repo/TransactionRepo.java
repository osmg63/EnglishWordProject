...
public interface TransactionRepo extends JpaRepository<TransactionWord, Integer> {
    ...
    List<Integer> findWordIdsByUserIdAndKnowTrue(int userId);
    @Query("SELECT t.word.id FROM TransactionWord t WHERE t.user.id = :userId ")
    List<Integer> findWordIdsByUserId(int userId);
